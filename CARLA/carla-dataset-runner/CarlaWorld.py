...
class CarlaWorld:
    def __init__(self, csv_file):
        ...
        self.num_seqs_recorded = 0
        self.data_path = "/scratch/vroom/data/CARLA_raw"
    ...
    def begin_data_acquisition(self, sensor_width, sensor_height, fov, frames_to_record_one_ego=100, timestamps=[], egos_to_run=10):
        ...
        with CarlaSyncMode(self.world, self.rgb_camera, self.depth_camera, fps=30) as sync_mode:
            ...
            while True:
                ...
                velocity3d = ego_vehicle.get_velocity()
                velocity = math.sqrt(velocity3d.x**2 + velocity3d.y**2 + velocity3d.z**2)
                if velocity > 0.01: # if below a threshold then skip 
                    sync_mode.tick_no_data()
                ...
                if not os.path.isdir(os.path.join(self.data_path, str(self.num_seqs_recorded))):
                    os.makedirs(os.path.join(self.data_path, str(self.num_seqs_recorded)))

                img_save_path = os.path.join(self.data_path, str(self.num_seqs_recorded), str(current_ego_recorded_frames)+".png")
                print("\nImg save path: ", img_save_path)
                cv2.imwrite(img_save_path, rgb_array)
                ...
                # normalized_steer = measurements.player_measurements.autopilot_control
                # steer_angle = normalized_steer * physics_control.wheels[0].max_steer_angle
                steer_angle = ego_vehicle.get_angular_velocity().z
                ...
                with open(self.csv_file, "a") as f: 
                    f.write(",".join([os.path.join(str(self.num_seqs_recorded), str(current_ego_recorded_frames)+".png"), str(timestamp), str(steer_angle)])+"\n")
